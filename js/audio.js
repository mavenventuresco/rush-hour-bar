...
export function setGameRunning(v) { _gameRunning = v; }
...
export function sfx(freq, dur, type = 'sine', vol = 0.15) {
  if (!AC || muted) return;
...
export function bellChime() {
  sfx(1200, 0.12, 'sine', 0.2);
  setTimeout(() => sfx(1600, 0.15, 'sine', 0.15), 100);
  setTimeout(() => sfx(2000, 0.20, 'sine', 0.10), 200);
}
export function serveSound()  { sfx(660, 0.08); setTimeout(() => sfx(880, 0.08), 70); setTimeout(() => sfx(1100, 0.12), 140); }
export function comboSound()  { [880, 1100, 1320, 1760].forEach((f, i) => setTimeout(() => sfx(f, 0.1, 'sine', 0.2), i * 55)); }
export function failSound()   { sfx(180, 0.15, 'sawtooth', 0.12); setTimeout(() => sfx(140, 0.18, 'sawtooth', 0.1), 120); }
export function tipSound()    { sfx(550, 0.06); setTimeout(() => sfx(730, 0.08), 60); }
export function clickSfx()    { sfx(400, 0.04, 'sine', 0.08); }
export function shakeSound()  { sfx(660, 0.05); setTimeout(() => sfx(880, 0.05), 60); }
